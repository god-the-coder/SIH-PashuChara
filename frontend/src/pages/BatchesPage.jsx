    ...
    <div className="relative min-h-screen w-full overflow-x-hidden text-[#1a1c18] dark:text-[#e8e4dc] flex justify-center bg-[#ede7db] dark:bg-[#050706] antialiased">
      ...
      <div className="relative z-10 w-full max-w-[430px] min-h-screen flex flex-col justify-between shadow-2xl bg-transparent border-x border-[#ded6c7] dark:border-[#1d221f]">
        ...
        <main className="p-4 space-y-3 flex-1 overflow-y-auto">
          <form onSubmit={handleResolveCode} className="p-3.5 rounded-2xl bg-white dark:bg-[#161c18] border border-[#ded5c2] dark:border-[#242824] shadow-sm space-y-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-xl border border-[#ded5c2] dark:border-[#242824] text-xs font-bold bg-[#faf7f0] dark:bg-[#0f1411] text-gray-800 dark:text-white outline-none"
              />
              <button
                type="submit"
            ...
            {resolvedBatch && (
              <div className="mt-2 p-3 rounded-xl bg-[#faf7f0] dark:bg-[#0f1411] border border-[#ded5c2] dark:border-[#242824] space-y-1.5">
                <p className="text-xs font-black text-[#064d2c] dark:text-white">{resolvedBatch.batch_label}</p>
                ...
                <button
                  ...
                  className="w-full mt-1 py-2 rounded-lg bg-emerald-50 dark:bg-[#161914] text-emerald-800 dark:text-[#86efac] text-xs font-bold border border-emerald-300 dark:border-emerald-700/50 hover:bg-emerald-100 dark:hover:bg-[#20402e] cursor-pointer"
                >
                  {t.reinspectBatchBtn}
                </button>
              </div>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <h3 className="text-sm font-black text-[#064d2c] dark:text-white">{batch.batch_label}</h3>
                ...
                {qrBatchId === batch.id && qrUrl && (
                  <div className="flex flex-col items-center gap-1.5 pt-2">
                    <img src={qrUrl} alt={`QR for ${batch.batch_code}`} className="w-32 h-32 rounded-xl border border-[#ded5c2] dark:border-[#242824]" />
