...
const STORAGE_CONDITIONS = [
  { value: "GOOD", label: "अच्छी (सूखी व सुरक्षित)" },
  { value: "FAIR", label: "ठीक-ठाक" },
  { value: "POOR", label: "खराब (नमी/खुली जगह)" },
];
...
export default function InspectionQuestionnairePage() {
  ...
  const [loadError, setLoadError] = useState(() =>
    inspectionId ? "" : "कोई सक्रिय जाँच नहीं मिली। कृपया नई जाँच शुरू करें।",
  );
  ...
  useEffect(() => {
    ...
    inspectionService
      ...
      .catch((apiError) => {
        if (!cancelled) setLoadError(apiError.message || "सवाल तैयार नहीं हो सके। कृपया पुनः प्रयास करें।");
      })
      .finally(() => {
  ...
  const handleUseLocation = () => {
    if (!navigator.geolocation) {
      ...
      showToast("इस डिवाइस पर स्थान सुविधा उपलब्ध नहीं है");
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        showToast("स्थान प्राप्त हुआ — मौसम जानकारी विश्लेषण में जोड़ी जाएगी");
      },
      () => {
        setLocationStatus("error");
        showToast("स्थान प्राप्त नहीं हो सका");
      },
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (questions.some((q) => !q.answer.trim())) {
      setSubmitError("कृपया सभी सवालों के जवाब दें");
      return;
    }
    ...
    try {
      ...
      setSubmitted(true);
      showToast("जवाब सुरक्षित हो गए! ✓");
    } catch (apiError) {
      setSubmitError(apiError.message || "जवाब सुरक्षित नहीं हो सके। कृपया पुनः प्रयास करें।");
    } finally {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setAnalyzeError(apiError.message || "विश्लेषण नहीं हो सका। कृपया पुनः प्रयास करें।");
    } finally {
  ...
  const handleSaveInspection = async () => {
    ...
    try {
      ...
      showToast("जाँच सुरक्षित हो गई! ✓");
    } catch (apiError) {
      setSaveError(apiError.message || "जाँच सुरक्षित नहीं हो सकी। कृपया पुनः प्रयास करें।");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden text-[#1a1c18] dark:text-[#f3ede2] flex justify-center bg-[#FAF7F0] dark:bg-[#121512] antialiased">
      <div className="relative z-10 w-full max-w-[430px] min-h-screen flex flex-col justify-between p-4 shadow-xl bg-[#FAF7F0] dark:bg-[#141814]">
        <SubPageHeader
          title="चारे की स्थिति की जानकारी"
          subtitle="AI द्वारा तैयार सवाल"
          backTo={-1}
        />

        {isLoadingQuestions && (
          <div className="flex-1 flex items-center justify-center">
            <p className="text-xs text-gray-500 dark:text-gray-400">AI आपकी तस्वीरों के आधार पर सवाल तैयार कर रहा है...</p>
          </div>
        )}
        ...
        {!isLoadingQuestions && !loadError && submitted && !result && (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center px-4">
            ...
            <p className="text-sm font-black text-[#14351d] dark:text-white">जवाब सुरक्षित हो गए!</p>
            <p className="text-xs text-gray-600 dark:text-gray-300">अब AI विश्लेषण शुरू करें।</p>
            ...
            >
              बाद में करें — डैशबोर्ड पर वापस जाएँ
            </button>
          </div>
        )}
        ...
        {!isLoadingQuestions && !loadError && result && (
          <div className="flex-1 overflow-y-auto px-1 py-3 space-y-3">
            ...
            <div className="p-4 rounded-2xl bg-white dark:bg-[#1a1f1a] border border-[#ded5c4] dark:border-[#2b352b] text-center space-y-1">
              ...
              <p className="text-sm font-black text-[#14351d] dark:text-white">विश्लेषण पूर्ण हुआ</p>
              <p className="text-[10px] text-gray-500 dark:text-gray-400">(अस्थायी दृश्य — पूर्ण रिपोर्ट पेज जल्द जुड़ेगा)</p>
            </div>
            <div className="p-4 rounded-2xl bg-white dark:bg-[#1a1f1a] border border-[#ded5c4] dark:border-[#2b352b] text-xs space-y-1.5">
              <p><span className="font-bold">जोखिम श्रेणी:</span> {result.risk_category}</p>
              <p><span className="font-bold">जोखिम स्कोर:</span> {result.risk_score ?? "-"}</p>
              <p><span className="font-bold">शीर्षक:</span> {result.headline}</p>
              <p><span className="font-bold">सुझाई गई कार्रवाई:</span> {result.action_label}</p>
              <p><span className="font-bold">सारांश:</span> {result.summary}</p>
              <p><span className="font-bold">विश्वास स्तर:</span> {result.confidence ?? "-"}%</p>
              <p><span className="font-bold">प्रयोगशाला जाँच आवश्यक:</span> {result.requires_lab_testing ? "हाँ" : "नहीं"}</p>
            ...
            {savedBatch ? (
              <div className="p-4 rounded-2xl bg-white dark:bg-[#1a1f1a] border border-emerald-500/40 text-center space-y-1">
                ...
                <p className="text-xs font-black text-[#14351d] dark:text-white">जाँच सुरक्षित व बैच से जुड़ी</p>
                {savedBatch.batch_code && (
                  <p className="text-xs font-mono font-bold text-emerald-800 dark:text-emerald-300">
                    बैच कोड: {savedBatch.batch_code}
                  </p>
                )}
              </div>
            ) : (
              <>
                ...
                >
                  {isSaving ? "सुरक्षित हो रहा है..." : "जाँच सुरक्षित करें 💾"}
                </button>
              </>
            )}
        ...
        {!isLoadingQuestions && !loadError && !submitted && (
          <form onSubmit={handleSubmit} className="my-3 space-y-3 flex-1 overflow-y-auto pr-0.5">
            ...
            <div className="bg-white dark:bg-[#1a1f1a] p-3.5 rounded-3xl border border-[#ded5c4] dark:border-[#2b352b] shadow-xs space-y-3">
              ...
              <div>
                <label className="block text-[11px] font-bold text-gray-700 dark:text-gray-300 mb-1">
                  क्या नमी का संपर्क हुआ है?
                </label>
                <div className="grid grid-cols-2 gap-2">
                  {[{ label: "हाँ", value: true }, { label: "नहीं", value: false }].map((opt) => (
                    <button
                      type="button"
              ...
              >
                {locationStatus === "done" ? "📍 स्थान प्राप्त हुआ ✓" : locationStatus === "locating" ? "स्थान प्राप्त हो रहा है..." : "📍 वर्तमान स्थान व मौसम जोड़ें"}
              </button>
            </div>
            ...
            <div className="pt-1 pb-2">
              ...
              >
                <span>{isSubmitting ? "सुरक्षित हो रहा है..." : "जवाब सुरक्षित करें"}</span>
